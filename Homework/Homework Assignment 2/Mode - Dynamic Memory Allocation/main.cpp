/* 
 * File:   main.cpp
 * Author: Dr. Mark E. Lehr modified by Sonya Smalley on 9/24/2021 at 10:01 AM
...
int *mode(const int *array,int arySize){
    ...
    int freq=1, maxFreq=1,curNum; //count frequency of each number, store max frequency, and track current number
    ...
    int nModes=0; //count how many modes there are
    freq=1;
    if (maxFreq>1)
    {
        for (int i=0;i<arySize;i++)
    ...
    //Fill the mode array
    int modeIdx=2; //position within the mode array
    for (int i=0;i<arySize;i++)
    {
        curNum=*(ary+i);
        if (curNum==*(ary+i+1))
