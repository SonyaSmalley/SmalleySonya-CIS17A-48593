 ...
 * Created on November 26, 2021, 11:40 AM
 * Purpose: Play the card game Hearts. Fix user input and output for tricks won
 */
...
void crdName(Card &card); //Name a card
...
short rndPlay(Player plyr[],short nPlyr,short sizeH,short first,
        short rnd,bool &brkn,fstream &tFile,fstream &bFile); //Play 1st round - 2C first
...
int main(int argc, char** argv) {
    ...
    fstream trkText,trkBin; //Store the tricks that are taken
    short *winner; //Track which players have the lowest score
    short minPts=26; //Determine the lowest points earned by a player
    char choice; //Hold the user's choice
    bool showAll; //Does the user want to see all hand and deck details?
    
    //Display an intro to the game
    intro();
    //Does the user want to see cards displayed?
    cout<<"\nWould you like the contents of the deck and of the other\n"
          "players' hands to be displayed to you as you play?\n"
          "Enter Y or N: ";
    cin>>choice;
    cin.ignore();
...
//*************************************************************************
    ...
    maxRnd=3;//for testing purposes, maxRnd is reduced
//*************************************************************************
    winner=new short[nPlayrs]; //Create an array of potential winners
    //Set winner array to 0s
    ...
    cout<<"The owner of the "<<((nPlayrs==4)?"Two":"Three")<<" of Clubs is "
            <<players[first].name<<".\n";
    ...
    //Display the tricks that the user won (pull them from binary file). Also
    //fill the winner array with the players' points
    trkBin.open("tricks.bin",ios::in | ios::out | ios::binary);
    cout<<"\nHere are all the tricks won in this game:\n";
    for (short p=0;p<nPlayrs;p++){
        //Display tricks
        for (short i=0;i<players[p].trkRnds.size();i++){
            if (i==0)
                cout<<players[p].name<<" won the trick for "
                    <<players[p].trkRnds.size()<<" rounds:\n";
            Card temp;
            cout<<"\tRound "<<players[p].trkRnds[i]<<": ";
            trkBin.seekg((sizeof(Card)*nPlayrs*(players[p].trkRnds[i]-1)),ios::beg);
            for (short r=0;r<nPlayrs;r++){
                trkBin.read(reinterpret_cast<char *>(&temp),sizeof(temp));
                cout<<temp.name;
                if (r!=nPlayrs-1)
                    cout<<", ";
            }
        ...
        if (players[p].trkRnds.size()>0){
            //Track player points
            cout<<"\tPoints earned: "<<players[p].points<<endl;
            winner[p]=players[p].points;
        }
        if (winner[p]<minPts)
            minPts=winner[p];
    }
    ...
    for (short i=0;i<nPlayrs;i++){
        if (winner[i]==minPts)
            cout<<players[i].name<<" won with the lowest points ("
                    <<minPts<<")!\n";
    }
...
Deck filDeck(short nPlyr){
    ...
    for (short i=0;i<tempDck.nCards;i++){
        ...
        tempDck.cards[i].order=i; //Note order in the deck
        tempDck.cards[i].value=i%13+2; //The face value
        //Assign Point values
        if (suit[i/13]==C || suit[i/13]==S || suit[i/13]==D){ //C,D,S = 0 pts
            tempDck.cards[i].point=0;
            if (suit[i/13]==S && face[i%13]=='Q') //except QS = 13 pts
                tempDck.cards[i].point=13;
        }
        else if (suit[i/13]==H) //H = 1 pt
            tempDck.cards[i].point=1;
        //Get the card name
        crdName(tempDck.cards[i]);
    }
...
//Input a Card.                       *
//******************************************************
void crdName(Card &card){
    char temp[SZNAME];
    //Get the face value name
    switch (card.face){
        case '2': strcpy(temp,"Two of "); break;
        case '3': strcpy(temp,"Three of "); break;
        case '4': strcpy(temp,"Four of "); break;
        case '5': strcpy(temp,"Five of "); break;
        case '6': strcpy(temp,"Six of "); break;
        case '7': strcpy(temp,"Seven of "); break;
        case '8': strcpy(temp,"Eight of "); break;
        case '9': strcpy(temp,"Nine of "); break;
        case 'T': strcpy(temp,"Ten of "); break;
        case 'J': strcpy(temp,"Jack of "); break;
        case 'Q': strcpy(temp,"Queen of "); break;
        case 'K': strcpy(temp,"King of "); break;
        case 'A': strcpy(temp,"Ace of "); break;
    }
    //Get the suit name
    switch (card.suit){
        case C: strcat(temp,"Clubs");break;
        case D: strcat(temp,"Diamonds");break;
        case S: strcat(temp,"Spades");break;
        case H: strcat(temp,"Hearts");break;
    }
    //Set the name
...
Player *getPlyr(short &nPlyr){
    cout<<"\nYou can play with 2, 3, or 4 other players.\nHow many other"
...
short dlPlayr(Player plyr[],short nPlyr,Deck cards,short sizeH){
    ...
    plyr[0].name=usrNm;
    plyr[0].points; //player starts with 0 points
    ...
    for (short i=1;i<nPlyr;i++){
        plyr[i].name=names[i];
        cout<<plyr[i].name<<" ";
        //Set player points to 0 to start with
        plyr[0].points=0;
    }
    cout<<endl;
...
short rndPlay(Player plyr[],short nPlyr,short sizeH,short first,short rnd,
            bool &brkn,fstream &tFile,fstream &bFile){    
    ...
    //Open files for recording the tricks in them and write the rnd # in the text file
    tFile.open("tricks.txt",ios::out | ios::app);
    bFile.open("tricks.bin",ios::out | ios::app);
    tFile<<"Round "<<rnd<<": ";
    ...
    for (short cnt=0;cnt<nPlyr;cnt++){
        ...
        //Write the card to the files
        tFile<<trck[cnt].name;
        if (cnt<nPlyr-1)
            tFile<<", ";
        bFile.write(reinterpret_cast<char *>(&trck[cnt]),sizeof(Card));
    ...
    plyr[first].points+=points; //keep track of player points
    plyr[first].trkRnds.push_back(rnd); //note which rounds a player wins tricks on
...
//Input a Player, short size of hand, short number of players, short *
//round, and short first.                                            *
//********************************************************************
short usrPkCd(Player plyr,short sizeH,short nPlyr,short rnd,short first){
    short pos; //to hold the indx of the card they chose
    char face,suit; //to hold the face and suit temporarily
    ...
    cin.clear();
    cin>>face>>suit;
    card.face=face=='t'?'T':face=='j'?'J':face=='q'?'Q':face=='k'?'K':
        face=='a'?'A':face;
    ...
    while (!match){
        if (rnd==1 && first==0){
            ...
            else if (nPlyr!=4 && (card.face=='3' && card.suit==C)){
                match=true;
                pos=0;
            }
            else if (nPlyr==4 && (card.face=='2' && card.suit==C)){
...
//***************************************************
//Display an intro to the game. Input bool show.    *
//***************************************************
void intro(){
    char choice; //User has the choice to read a short version of the rules
    ...
    if (choice=='y' || choice=='Y'){
        ...
        "and all players' hands may be displayed during the game.\n"
